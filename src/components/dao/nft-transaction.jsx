/**
 * DAO GOVERNANCE NFT STAKING TRANSACTION COMPONENT
 *
 * Page: /plr-dao-staking
 * Purpose: Stake PLR to receive DAO membership NFT
 *
...
Required stake amount (as of Nov 1 2024): 10000 PLR
...
// Contract configuration - auto-switches between mainnet/testnet based on VITE_USE_TESTNET
const polygonChainId = contractConfig.chainId;
const daoContractAddress = contractConfig.daoContract;
const tokenAddress = contractConfig.tokenContract;
const stakeToken = contractConfig.stakeAmount;
const stakeTokenAmount = ethers.utils.parseUnits(stakeToken, 18);
const explorer = contractConfig.explorer;
const networkName = contractConfig.networkName;
const minStakeTimeSeconds = contractConfig.minStakeTime || 180; // 6 seconds for testnet, 180 for mainnet
...
const DaoMemberNftTx = ({ onLogout }) => {
  ...
  const shouldShowUnstakeButton = useMemo(() => {
    ...
    return currentTimeInSeconds > membershipTime + minStakeTimeSeconds;
  }, [membershipTime]);
  ...
  const handleWriteDeposit = async () => {
    writeDeposit(
      {
  ...
  useEffect(() => {
    ...
    function changeButton() {
      if (!isUsingPolygon) {
        ...
        const currentApproveStatus = approveStatus;
        const currentDepositStatus = depositStatus;

        if (currentApproveStatus === 'idle' || currentApproveStatus === 'error') {
          setButtonText('Approve PLR');
          setButtonEnabled(true);
        }
        if (currentApproveStatus === 'pending') {
          setButtonText('Approval Pending');
          setButtonEnabled(false);
        }
        if (currentApproveStatus === 'success') {
        ...
        if (currentDepositStatus === 'success') {
  ...
  return (
    <Wrapper>
      <WrapperTitle>Send Transaction on {networkName}</WrapperTitle>
      <ButtonWrapper>
        {isAlreadyMember && (
          <>
        ...
        {/* Network switch button - show for members on wrong network OR non-members */}
        {(!isAlreadyMember || !isUsingPolygon) && (
          <TransactionButton
            id="switchToPolygonNetwork"
            disabled={isUsingPolygon || isSwitching}
        ...
        {isAlreadyMember && shouldShowUnstakeButton && (
          <>
            <DaoUnstakeButton chainId={polygonChainId} contract={daoContractAddress} explorer={explorer} networkName={networkName}></DaoUnstakeButton>
          </>
        )}
        {!isAlreadyMember && (
          <>
            ...
            {(isDepositTxError || isApproveTxError) && errorText && (
              <TxInfo>
                <div id="errorMessage">
